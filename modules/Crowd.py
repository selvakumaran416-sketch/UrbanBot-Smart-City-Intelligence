...
class Config:
    ...
    UPLOAD_DIR = "uploads/crowd_status"
    # Replace with your actual road damage detection model path
    MODEL_PATH = "C:/Users/Selva.M/Downloads/data_science/final_project/models/crowd.pt"
...
class EmergencyService:
    ...
    def trigger_sos(city, area, severity):
        ...
        subject = f"🚨 URGENT: {severity} CROWD DETECTED IN {area.upper()}, {city.upper()}"
        body = f"""
        EMERGENCY ALERT: CROWD DETECTED
        ------------------------------------------
        Location: {area}, {city}
...
class DatabaseManager:
    """Logs crowd density data for urban planning analytics."""
    @staticmethod
        ...
        try:
            ...
            with mysql.connector.connect(**Config.DB_CONFIG) as conn:
                with conn.cursor() as cursor:
                    query = """INSERT INTO crowd_reports 
                               (city, area, latitude, longitude, people_count, severity, image_name) 
...
def main():
    st.set_page_config(page_title="Crowd_Detection AI", layout="wide", page_icon="🚦")
    st.title("🚦Smart City: Real-Time Crowd Density Detection System")
    st.markdown("---")
    ...
    mode = st.sidebar.radio("Input Source", ["Static Image", "Live Crowd Feed"])
    ...
    if mode == "Static Image":
        with col_input:
            ...
            if uploaded_file and st.button("🚀 Analyze Crowd", width="stretch"):
                ...
                with col_output:
                    st.image(results[0].plot(), channels="BGR", caption="Crowd Snapshot")
                    st.metric("People Counted", len(detections))
    ...
    # --- MODE: LIVE CROWD FEED ---
    else:
        with col_input:
            st.info("Accessing Live Camera...")
        ...
        if run:
            ...
            if 'last_crowd_log' not in st.session_state:
                st.session_state.last_crowd_log = datetime.now()
            ...
            while run:
                ...
                if "CRITICAL" in severity or "HEAVY" in severity:
                    status_box.error(f"🚨 Crowd Detected at {area}!")
                    ...
                    if (current_time - st.session_state.last_alert_time).total_seconds() > 20:
                        ...
                        EmergencyService.trigger_sos(city, area, severity)
                        st.session_state.last_crowd_log = current_time
                        st.toast("Emergency services notified!",icon="🚑")
                else:
                    status_box.success("✅ Crowd Detection Flowing Normal")
            cap.release()

    # ================= FOOTER =================
    st.divider()
    st.markdown(
        "<p style='text-align: center; color: grey;'>© Selvakumaran Muthusamy | Crowd Detection Page</p>",
