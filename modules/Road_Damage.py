...
class Config:
    ...
    MODEL_PATH = "C:/Users/Selva.M/Downloads/data_science/final_project/models/pathole_model.pt"
...
class EmergencyService:
    ...
    def trigger_sos(city, area, severity):
        ...
        subject = f"🚨 URGENT: {severity} ROAD DAMAGE DETECTED IN {area.upper()}, {city.upper()}"
        body = f"""
        EMERGENCY ALERT: ROAD DAMAGE DETECTED
        ------------------------------------------
        Location: {area}, {city}
...
class DatabaseManager:
    """Logs road damage density data for urban planning analytics."""
    @staticmethod
        ...
        try:
            ...
            with mysql.connector.connect(**Config.DB_CONFIG) as conn:
                with conn.cursor() as cursor:
                    query = """INSERT INTO road_damage_reports 
                               (city, area, latitude, longitude, damage_count, severity, image_name) 
...
# =================================================================
def main():
    st.set_page_config(page_title="RoadGuard Pro AI", layout="wide")
    st.title("🛣️ Smart City: Real-Time Road Damage Detection System")
    st.markdown("---")
    ...
    mode = st.sidebar.radio("Input Source", ["Static Image", "Live Camera Feed"])
    ...
    # --- MODE: LIVE CAMERA FEED ---
        ...
        if run:
            ...
            while run:
                ...
                if "CRITICAL" in severity or "HEAVY" in severity:
                    status_box.error(f"🚨 Road Damage Detected at {area}!")
                    ...
                    status_box.success("✅ No Road Damage Detected")
            cap.release()

    # ================= FOOTER =================
    st.divider()
    st.markdown(
        "<p style='text-align: center; color: grey;'>© Selvakumaran Muthusamy | Roade Damage Detection Page</p>",
